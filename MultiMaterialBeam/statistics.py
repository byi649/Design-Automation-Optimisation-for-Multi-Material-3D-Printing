...
def runHeuristic():
    ...
    popArray = [40]
    f1Array = [None, 100, 300]
    gradArray = [None, 50, 400]
    ...
    for i, config in enumerate(list(itertools.product(popArray, f1Array, gradArray))):
        ...
        start = time.time()
        (bin, fbest, best) = algos.GA_voxel_uniform(verbose, NGEN, nVoxels, config[0], config[1], config[2])
        end = time.time()
    ...
    data = {'fbest': fbestlist,'sol': sollist, 'freq': freqlist, 'time': timelist, 'nPop': poplist, 'f1': f1list, 'grad': gradlist, 'f1_true': f1truelist, 'grad_true': gradtruelist}
